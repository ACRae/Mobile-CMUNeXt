...
def low_pass_filter2(image, r=8):
    f = image
    f_compress = torch.complex(
        torch.zeros(in_channels, image_size, image_size // 2),
        torch.zeros(in_channels, image_size, image_size // 2),
    ).to(device)
    for i in range(in_channels):
        f_compress[i, :r, :r] = f[i, :r, :r]
...
def low_pass_filter3(image, r=8):
    f = image
    f_compress = torch.complex(
        torch.zeros(f.size(0), 1, image_size, image_size // 2, 2),
        torch.zeros(f.size(0), 1, image_size, image_size // 2, 2),
    ).to(device)
...
def low_pass_filter4(image, r=8):
    f = image
    f_compress = torch.complex(
        torch.zeros(f.size(0), 1, image_size, image_size // 2),
        torch.zeros(f.size(0), 1, image_size, image_size // 2),
    ).to(device)
    f_compress[:, :, :r, :r] = f[:, :, :r, :r]
    f_compress[:, :, -r:, :r] = f[:, :, -r:, :r]
...
class ZeroPadding(Module):
    ...
    def forward(self, f):
        ...
        f_compress = torch.complex(
            torch.zeros(f.size(0), 1, self.size, self.size),
            torch.zeros(f.size(0), 1, self.size, self.size),
        )
        f_compress[:, :, :r, :r] = f[:, :, :r, :r]
        f_compress[:, :, -r:, :r] = f[:, :, -r:, :r]
