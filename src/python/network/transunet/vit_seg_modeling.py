...
class DecoderCup(nn.Module):
    def __init__(self, config):
        ...
        blocks = [
            ...
            for in_ch, out_ch, sk_ch in zip(in_channels, out_channels, skip_channels, strict=False)
        ]
    ...
    def forward(self, hidden_states, features=None):
        ...
        for i, decoder_block in enumerate(self.blocks):
            skip = (features[i] if i < self.config.n_skip else None) if features is not None else None
...
class VisionTransformer(nn.Module):
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            # Encoder whole
            for _bname, block in self.transformer.encoder.named_children():
