...
from utils.params import calculate_macs_and_params
...
def setup(device, config):
    ...
    macs, params = calculate_macs_and_params(
        config["input_channels"],
        config["input_w"],
        config["input_h"],
        model=networks.get_model(config["model"], config).to(device),  # temporary model
        device=device,
    )
...
def main():
    ...
    for epoch in range(config["epochs"]):
        ...
        if val_log["val_iou"] > best_iou:
            ...
            best_log = {
                "epoch": epoch,
                "val_loss": float(log["val_loss"][-1]),
                "val_iou": float(log["val_iou"][-1]),
                "val_dice": float(log["val_dice"][-1]),
                "PC": float(log["PC"][-1]),
                "F1": float(log["F1"][-1]),
                "AAC": float(log["AAC"][-1]),
            }
            torch.save(model.state_dict(), os.path.join(config["model_dir"], "model.pth"))
            with open(os.path.join(config["model_dir"], "best_metrics.yml"), "w") as f:
                yaml.dump(best_log, f, sort_keys=False)
