...
from .common import ConvNormAct, _make_divisible
...
class Involution(nn.Module):
    ...
    def forward(self, x):
        ...
        x_unfolded = x_unfolded.view(
            B,
            self.groups,
            torch.div(C, self.groups, rounding_mode="floor"),
        ...
        kernel = kernel.view(B, self.groups, self.kernel_size * self.kernel_size, H, W).unsqueeze(2)
