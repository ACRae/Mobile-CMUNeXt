...
from network.mobile_cmunext.quantized.modules import (
    ActQuant,
    BiasQuant,
    QuantBatchNorm2d,
    QuantHardswish,
...
class Residual(nn.Module):
    ...
    def forward(self, x):
        fn_x = self.input_quant(self.fn(x))
        ...
        self.block = nn.Sequential(
            *[
                nn.Sequential(
                    ...
                    QuantBatchNorm2d(
                        ch_in * 4, ActQuant, bit_width=get_act_bit_width(), return_quant_tensor=True
                    ),
                    qnn.QuantConv2d(
                        in_channels=ch_in * 4,
                        out_channels=ch_in,
                    ...
                    QuantBatchNorm2d(
                        ch_in, ActQuant, bit_width=get_act_bit_width(), return_quant_tensor=True
                    ),
                )
...
class ConvBlock(nn.Module):
    def __init__(self, ch_in, ch_out):
        ...
        self.conv = nn.Sequential(
            ...
            QuantBatchNorm2d(ch_out, ActQuant, bit_width=get_act_bit_width(), return_quant_tensor=True),
            QuantHardswish(
                act_quant=ActQuant,
                bit_width=get_act_bit_width(),
                return_quant_tensor=True,
...
class FusionConv(nn.Module):
    def __init__(self, ch_in, ch_out):
        ...
        self.conv = nn.Sequential(
            ...
            QuantBatchNorm2d(ch_out * 4, ActQuant, bit_width=get_act_bit_width(), return_quant_tensor=True),
            qnn.QuantConv2d(
...
class MobileCMUNeXt_Quant(nn.Module):
    def __init__(
        self,
        weight_bit_width,
        act_bit_width,
        ...
        self.encoder1 = MobileCMUNeXtBlock(ch_in=dims[0], ch_out=dims[0], depth=depths[0], k=kernels[0])
        self.encoder2 = MobileCMUNeXtBlock(ch_in=dims[0], ch_out=dims[1], depth=depths[1], k=kernels[1])
        self.encoder3 = MobileCMUNeXtBlock(ch_in=dims[1], ch_out=dims[2], depth=depths[2], k=kernels[2])
        self.encoder4 = MobileCMUNeXtBlock(ch_in=dims[2], ch_out=dims[3], depth=depths[3], k=kernels[3])
        self.encoder5 = MobileCMUNeXtBlock(ch_in=dims[3], ch_out=dims[4], depth=depths[4], k=kernels[4])
        # Decoder
        self.Up5 = UpConv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = FusionConv(ch_in=dims[3], ch_out=dims[3])
        self.Up4 = UpConv(ch_in=dims[3], ch_out=dims[2])
        self.Up_conv4 = FusionConv(ch_in=dims[2], ch_out=dims[2])
        self.Up3 = UpConv(ch_in=dims[2], ch_out=dims[1])
        self.Up_conv3 = FusionConv(ch_in=dims[1], ch_out=dims[1])
        self.Up2 = UpConv(ch_in=dims[1], ch_out=dims[0])
        self.Up_conv2 = FusionConv(ch_in=dims[0], ch_out=dims[0])
